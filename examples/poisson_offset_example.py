...
offset = np.random.randint(1, 5, (n, 1)) / 1.  # offset must be float
lam = offset * np.exp(X @ b.T)
y = np.random.poisson(lam, (n, 1))

skinny_model = skinny.glm.SkinnyGLM(family=skinny.families.PoissonFamily(link=skinny.links.LogLink()))
