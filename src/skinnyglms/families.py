...
class GaussianFamily(BaseFamily):
    ...
    def sample(self, theta: np.ndarray, phi: Union[int, np.ndarray]=1, n: int=1):
        ...
        scale = phi
        return np.hstack([np.random.normal(loc=loc, scale=scale, size=(theta.shape[0], 1)) for _ in range(n)])
...
class BinomialFamily(BaseFamily):
    ...
    def sample(self, theta: np.ndarray, phi: Union[int, np.ndarray]=1, n: int=1):
        ...
        return np.hstack([np.random.binomial(n=1, p=p, size=(theta.shape[0], 1)) for _ in range(n)])
...
class PoissonFamily(BaseFamily):
    ...
    def sample(self, theta: np.ndarray, phi: Union[int, np.ndarray]=1, n: int=1):
        ...
        return np.hstack([np.random.poisson(lam=lam, size=(theta.shape[0], 1)) for _ in range(n)])